...
class AccountInvoiceDianDocument(models.Model):
    ...
    def _get_xml_values(self, ClTec):
        ...
        return {
            ...
            'Note3': self.invoice_id.amount_total or 0.0,
            'Note4': '\n' + (self.invoice_id.narration2 or ''),
            'Note5': self.invoice_id.invoice_payment_term_id.name, # invoice_date_due.strftime("%d/%m/%Y") or '',
            'Note6': sum([x.quantity for x in self.invoice_id.invoice_line_ids.filtered(lambda l: not l.display_type)]),
            'Note7': '{:.2f}'.format(RteFte),
            'Note8': '{:.2f}'.format(RteIVA),
            'Note9': '{:.2f}'.format(RteICA),
